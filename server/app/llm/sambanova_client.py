"""
SambaNova LLM Client — OpenAI-compatible API at https://api.sambanova.ai/v1
Env var: SAMBANOVA_API_KEY (JSON array of keys)
"""
import asyncio
import logging
...
from app.llm.base_client import BaseClient

logger = logging.getLogger(__name__)

SAMBANOVA_BASE_URL = "https://api.sambanova.ai/v1"
SAMBANOVA_DEFAULT_MODEL = "DeepSeek-V3.2"

_SENTINEL = object()


class SambaNovaClient(BaseClient):
    """SambaNova provider — 200K TPM, good for long content generation."""
    ...
    def _create_client(self, api_key: str) -> Any:
        return OpenAI(api_key=api_key, base_url=SAMBANOVA_BASE_URL, timeout=60.0, max_retries=1)
    ...
    async def _do_chat(self, client: Any, messages: List[Dict[str, str]], model: str, temperature: float, max_tokens: int) -> str:
        def _call() -> str:
            ...
            if not completion.choices:
                raise ValueError("SambaNova returned empty choices")
