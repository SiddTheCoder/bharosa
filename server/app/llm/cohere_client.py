"""
Cohere LLM Client — OpenAI-compatible API at https://api.cohere.ai/compatibility/v1
Env var: COHERE_API_KEY (JSON array of keys)
"""
import asyncio
import logging
...
class CohereClient(BaseClient):
    ...
    def _create_client(self, api_key: str) -> Any:
        return OpenAI(api_key=api_key, base_url=COHERE_BASE_URL, timeout=30.0, max_retries=1)
    ...
    async def _do_chat(self, client: Any, messages: List[Dict[str, str]], model: str, temperature: float, max_tokens: int) -> str:
        def _call() -> str:
            ...
            if not completion.choices:
                raise ValueError("Cohere returned empty choices")
