"""
Cerebras LLM Client — OpenAI-compatible API at https://api.cerebras.ai/v1
Env var: CEREBRAS_API_KEY (JSON array of keys)
"""
import asyncio
import logging
...
class CerebrasClient(BaseClient):
    """Cerebras provider — fast inference, 2400 RPD free tier."""
    ...
    async def _do_chat(self, client: Any, messages: List[Dict[str, str]], model: str, temperature: float, max_tokens: int) -> str:
        def _call() -> str:
            ...
            if not completion.choices:
                raise ValueError("Cerebras returned empty choices")
