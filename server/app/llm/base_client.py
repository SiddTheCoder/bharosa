...
Features:
- Reads keys from the in-memory env-only key store (app.llm.keys)
- JSON array key parsing (e.g. '["key1","key2"]')
...
# Key rotation backed by the env-only in-memory store.
...
class BaseClient(ABC):
    ...
    def _load_keys(self) -> List[str]:
        """Load API keys for this provider from the env-only in-memory store."""
        keys = get_all_keys(self.provider_name.lower())

        # Also try with env_key if provider name didn't resolve.
        if not keys:
            keys = get_all_keys(self.env_key)

        # Last-resort direct env read.
        if not keys:
